...
box.style.cssText='display:grid;grid-template-columns:repeat('+n+',1fr);gap:6px;margin:8px 0';
ids.forEach(function(id){
getPhoto(id).then(function(rec){
if(rec&&rec.dataUrl){
const im=document.createElement('img');
im.src=rec.dataUrl;
im.style.cssText='width:100%;aspect-ratio:1;object-fit:cover;border-radius:10px;display:block;cursor:pointer';
...
let h='<div class="wt-card"><div class="wt-title">Последние тесты воды</div>';
h+='<div class="wt-gridcards" id="wtGrid"></div></div>';
h+='<div class="wt-card" id="wtGraphCard">';
h+='<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px">';
h+='<div class="wt-title" style="margin-bottom:0">График тестов</div>';
h+='<select id="chartMonthSel" style="background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:10px;color:#e0f0ff;font-size:12px;padding:6px 8px;max-width:46%"></select></div>';
h+='<div class="wt-chips" id="wtChips"></div><div id="wtChart"></div></div>';
wrap.innerHTML=h;
cv.appendChild(wrap);
